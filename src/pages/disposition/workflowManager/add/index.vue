...
<template>
  <section class="system-page-slot workflow-manager-add">
    <!-- TODO: form 補相關屬性 -->
    <el-form :model="formData.form">
      <div class="workflow-manager-add__header">
        <div class="workflow-manager-add__header-title">
          <span class="material-symbols-rounded" aria-hidden="true">docs</span>
          <h1 class="workflow-manager-add__title">新增處理程序</h1>
        </div>
        <div class="workflow-manager-add__header-actions">
      ...
      <div class="workflow-manager-add__form-panel">
        <system-form-list>
      ...
      <div class="workflow-manager-add__process">
        <div>
          ...
          <div class="workflow-manager-add__step-list">
            <template v-for="(item, index) in formData.form.processingFlow" :key="item.seq">
              <div
                :class="[
                  'workflow-manager-add__step',
                  {
                    'workflow-manager-add__step--active': formData.currentStep.seq === item.seq
                  }
                ]"
                @click="handleStepClick(item)"
              >
                <span
                  class="material-symbols-rounded workflow-manager-add__step-icon"
                  aria-hidden="true"
                >
                  drag_indicator
                </span>
                <div class="workflow-manager-add__step-body">
                  <span class="workflow-manager-add__step-seq">Step{{ item.seq }}</span>
                  <span class="workflow-manager-add__step-title">{{ item.title }}</span>
                </div>
              </div>
              <div
                v-if="index < formData.form.processingFlow.length - 1"
                class="workflow-manager-add__step-connector"
                aria-hidden="true"
              >
                ↓
              </div>
        ...
        <div class="workflow-manager-add__step-editor">
          <system-form-list>
            <div class="system-form__row system-form__row--full">
              <div class="workflow-manager-add__step-editor-header">
                <span class="workflow-manager-add__step-editor-seq">
